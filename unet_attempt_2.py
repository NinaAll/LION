...
train_param.optimiser = "adam"  # keeps track of how big the previous steps were (version of gradient descent with momentum), Nesterow (?) descent <- nice paper!
# adam version of Nesterow descent, but better

# optimizer
train_param.epochs = 100
train_param.learning_rate = (
    1e-4  # always changeable to see if the code trains: e.g., 1e-4
)
...
for epoch in range(start_epoch, start_epoch + train_param.epochs):
    train_loss = 0.0
...
plt.semilogy(x, loss_valid, label="valid")
plt.legend()
...
# nohup python unet_attempt_2.py &
# tail -f nohup.out
