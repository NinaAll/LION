...
class smoothed_net(nn.Module):
        ...
        if self.smooth_length == 0:
            self.wxs = nn.ModuleList(
                [
                    ...
                    for _ in range(self.n_layers + 1 - self.smooth_length)
                ]
                + [
                    nn.Conv2d(
                        n_channels,
                    ...
                    for _ in range(
                        self.n_layers + 1 - self.smooth_length, self.n_layers + 1
                    )
                ]
            )
        else:
            self.wxs = nn.ModuleList(
                [
                    ...
                    for _ in range(self.smooth_length)
                ]
                + [
                    nn.Conv2d(
                        n_channels,
                    ...
                    for _ in range(self.smooth_length, self.n_layers + 1)
                ]
            )
    ...
    def wei_dec(self):
        rate = 10
        for i in range(self.n_kernels):
            self.conv[i].weight.data = (1 - 2 * rate * self.args.lr) * self.conv[
                i
            ].weight.data
    ...
    def forward(self, x, grady=False):
        ...
        for layer_idx in range(self.n_layers):
            if layer_idx < self.smooth_length - 1:
                ...
                z = self.leaky_relu(
                    self.wzs[layer_idx](z) + self.wxs[layer_idx + 1](z_sm)
                )
        z = self.final_conv2d(z)
        net_output = z.view(z.shape[0], -1).mean(dim=1, keepdim=True)
        assert net_output.shape[0] == x.shape[0], f"{net_output.shape}, {x.shape[0]}"
...
class Algorithm(BaseAlg.baseNet):
    def __init__(self, args, data_loaders, path=config.data_path + "nets/"):
        ...
        super(Algorithm, self).__init__(
            args, path, MyNet(args, n_chan=n_chan), data_loaders
        )
        self.eps = args.eps
    ...
    def lamb_approx(self):
        ...
        if config.angles != 0:
            gradient_truth = config.fwd_op_adj_mod((config.fwd_op_mod(truth) - scans))
        else:
            gradient_truth = truth - scans
        lambdy = np.mean(
            np.sqrt(np.sum(np.square(gradient_truth.numpy()), axis=(1, 2, 3)))
        )
    ...
    def grady(self, x):
        ...
        fake = Variable(
            torch.Tensor(a.shape[0], 1).fill_(1.0).type_as(x), requires_grad=False
        )
    ...
    def loss(self, scans, truth):
        ...
        decay_loss = 0
        loss = (
            self.net(real_samples).mean()
            - self.net(fake_samples).mean()
    ...
    def output(self, scans, truth=None, lambd=0):
        ...
        for j in range(self.args.iterates):
            ...
            if truth is not None:
                ...
                writer.add_scalar(
                    "Grad_size_ratio",
                    gradsizel2_fidelity.item() / gradsizel2_regulariser.item(),
                    j,
                )
                writer.add_scalar("Grad_size_l2", gradsizel2.item(), j)
                writer.add_scalar(
                    "Grad_size_l2_fidelity", gradsizel2_fidelity.item(), j
                )
                writer.add_scalar(
                    "Grad_size_l2_regulariser", gradsizel2_regulariser.item(), j
                )
            optimizer.step()
        writer.close()
        return guess
    ...
    def train(self, writer, epoch):
        self.net.train()
        for i, ((scans_1, truth_1), (_, truth_2)) in enumerate(
            zip(self.data_train_loader, self.data_train_loader)
        ):
            ...
            writer.add_scalar(
                "Loss", loss, (epoch - 1) * len(self.data_train_loader) + i
            )
            if i % self.args.log_interval == 0:
                print(
                    str(self.args.expir)
                    + ":Train Epoch: {} [{}/{} ({:.0f}%)] Loss: {:.6f}, Time: {:.6f}s".format(
                        epoch,
                        i * len(scans_1),
                        len(self.data_train_loader.dataset),
                        100.0 * i / len(self.data_train_loader),
                        loss,
                        end - start,
                    )
                )
                with open(
                    "./logs/" + self.args.alg + str(self.args.expir) + ".txt", "a"
                ) as f:
                    f.write(
                        "Train Epoch: {} [{}/{} ({:.0f}%)] Loss: {:.6f}, Time: {:.6f}s\n".format(
                            epoch,
                            i * len(scans_1),
                            len(self.data_train_loader.dataset),
                            100.0 * i / len(self.data_train_loader),
